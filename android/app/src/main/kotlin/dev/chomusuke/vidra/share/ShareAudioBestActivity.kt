...
class ShareAudioBestActivity : Activity() {
    companion object {
        private const val TAG = "ShareAudioBestActivity"
    }
    ...
    private fun handleShare() {
        ...
        val payload = ShareIntentParser.parse(
            ...
            presetId = SharePresetIds.AUDIO_BEST,
            directShare = true,
        )
        ...
        Toast.makeText(this, R.string.share_pending_audio_best, Toast.LENGTH_SHORT).show()
        finish()
    }
    ...
    private fun launchVidraApp(payload: ShareIntentPayload) {
        ...
        val launchIntent = Intent(Intent.ACTION_SEND).apply {
            setClass(this@ShareAudioBestActivity, MainActivity::class.java)
            type = "text/plain"
            putExtra(Intent.EXTRA_TEXT, shareText)
            if (!payload.subject.isNullOrBlank()) {
                putExtra(Intent.EXTRA_SUBJECT, payload.subject)
            }
            putExtra(MainActivity.EXTRA_SHARE_PRESET, SharePresetIds.AUDIO_BEST)
