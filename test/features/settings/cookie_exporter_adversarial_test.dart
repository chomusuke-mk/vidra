...
void main() {
  group('NetscapeCookieFormatter Adversarial Edge Cases', () {
    test('IP Addresses: IPv4, IPv6, and bracketed IPv6 formats', () {
      final cookies = [
        ...
        Cookie(name: 'c_ipv6_full', value: '5', domain: '2001:0db8:85a3:0000:0000:8a2e:0370:7334'),
        Cookie(name: 'c_ipv6_bracketed', value: '6', domain: '[::1]'),
        Cookie(name: 'c_ipv6_linklocal', value: '7', domain: 'fe80::1ff:fe23:4567:890a'),
    ...
    test('Domain Edge Cases: Multi-level subdomains, ports, and empty fallbacks', () {
      final cookies = [
        Cookie(name: 'c_deep', value: 'v1', domain: 'a.b.c.d.example.co.uk'),
        Cookie(name: 'c_dot_prefix', value: 'v2', domain: '.music.youtube.com'),
        Cookie(name: 'c_spaces', value: 'v3', domain: '   .sub.domain.org   '),
      ...
      final result = NetscapeCookieFormatter.format(
        cookies,
        defaultDomain: 'fallback-hub.com',
      );
    ...
    test('Domain Fallback when defaultDomain is also blank defaults to localhost', () {
      final cookies = [
    ...
    test('Expiration Edge Cases: 0, negative, Year 2038, Year 9999, and int64 max', () {
      final cookies = [
        ...
        Cookie(name: 'c_year_9999_ms', value: '10', expiresDate: 253402300799000),
        // Extreme 64-bit int max
        Cookie(name: 'c_int64_max_ms', value: '11', expiresDate: 9223372036854775807),
      ];
      ...
      expect(lines[11].split('\t')[4], equals('1780000000')); // ms converted to sec
      expect(lines[12].split('\t')[4], equals('25340230079')); // Y9999 sec
      expect(lines[13].split('\t')[4], equals('253402300799')); // Y9999 ms
      expect(lines[14].split('\t')[4], equals('9223372036854776')); // int64 max / 1000
    });

    test('Special Characters in Values: JSON, Base64, URL encoding, Unicode & Emojis', () {
      final cookies = [
        Cookie(
          name: 'auth_jwt',
          value: 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIn0.doNotLeak',
        ),
        Cookie(
          name: 'base64_data',
        ...
        Cookie(
          name: 'json_payload',
          value: '{"user_id": 9999, "session_active": true, "flags": ["premium", "beta"]}',
        ),
        Cookie(
          name: 'url_encoded_param',
          value: 'query=flutter+inappwebview&filter=%7B%22date%22%3A%22today%22%7D',
        ),
        Cookie(
      ...
      for (int i = 4; i < 10; i++) {
        ...
        expect(cols.length, equals(7), reason: 'Line $i should have exactly 7 TSV columns');
      }

      expect(lines[4].split('\t')[6], equals('eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIn0.doNotLeak'));
      expect(lines[5].split('\t')[6], equals('aHR0cHM6Ly95b3V0dWJlLmNvbS93YXRjaD92PWRRd3c0dzlXZ1hjUQ=='));
      expect(lines[6].split('\t')[6], equals('{"user_id": 9999, "session_active": true, "flags": ["premium", "beta"]}'));
      expect(lines[7].split('\t')[6], equals('query=flutter+inappwebview&filter=%7B%22date%22%3A%22today%22%7D'));
      expect(lines[8].split('\t')[6], equals('!@#\$%^&*()-_=+[]{}|;:\'",.<>/?`~'));
      expect(lines[9].split('\t')[6], equals('🍪_cookies_delicieux_日本語_русский_español_🔥'));
    });
    ...
    test('Large Batches (1,500+ cookies): Benchmarks formatting throughput and column integrity', () {
      ...
      final cookies = List.generate(
        ...
        (i) => Cookie(
          ...
          domain: (i % 3 == 0) ? 'domain$i.net' : ((i % 3 == 1) ? '.sub$i.org' : '10.0.0.$i'),
      ...
      final lines = result.trim().split('\n');
      expect(lines.length, equals(4 + batchSize)); // 4 headers + 1,500 cookies

      for (int i = 0; i < batchSize; i++) {
        final cols = lines[4 + i].split('\t');
        expect(cols.length, equals(7), reason: 'Cookie line index $i must have 7 columns');
  ...
  group('CookieExporter Adversarial File I/O and Permissions', () {
    ...
    setUp(() async {
      tempDir = await Directory.systemTemp.createTemp('vidra_exporter_adversarial_');
    });
    ...
    test('Large Batch Export (1,000 cookies) persists and matches exact TSV content', () async {
      final cookies = List.generate(
        1000,
        (i) => Cookie(
          name: 'bulk_cookie_$i',
    ...
    test('Deeply nested non-existent directory creation (10 levels deep)', () async {
    ...
    test('Permission & Invalid Path Stress: Throws FileSystemException when parent is a regular file', () async {
      ...
      expect(
        () async => await CookieExporter.saveCookiesToFile(
          cookies,
          defaultDomain: 'fail.com',
          targetPath: impossiblePath,
        ),
        throwsA(isA<FileSystemException>()),
      );
    });

    test('Concurrent Exports: Multiple parallel writes complete safely and produce valid file', () async {
      ...
      final defaultFilePath = p.join(tempDir.path, CookieExporter.defaultCookieFileName);
